...
export default function LoginClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data } = await api.get<{ status: string; data?: { maintenanceMode?: boolean } }>('/maintenance-status');
        if (!cancelled && data?.data?.maintenanceMode) setMaintenanceNotice(true);
      } catch {
        // If we can't reach the API, show login (fail open)
      }
    })();
              ...
              {maintenanceNotice && (
                ...
                >
                  <p style={{ color: 'var(--text-secondary)' }}>
                    System is under maintenance. Only administrators can sign in.
                  </p>
                </div>
              )}
